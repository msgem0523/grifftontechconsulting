...
export function Chip({tone='neutral',size='m',selected=false,children,style,...rest}){
  const [bg,fg,bd]=TONES[tone]||TONES.neutral;
  return <span style={{display:'inline-flex',alignItems:'center',gap:6,
    padding:size==='s'?'4px 12px':'7px 16px',fontSize:size==='s'?12:13,lineHeight:'18px',letterSpacing:'-0.1px',
    fontFamily:'var(--font-body)',fontWeight:500,borderRadius:'var(--radius-pill)',
    background:selected?'var(--ink)':bg,color:selected?'var(--white)':fg,
    border:'1px solid '+(selected?'var(--ink)':bd),whiteSpace:'nowrap',
